...
export function normalizeAndValidateCompanyEmail(email: string): string | null {
  const normalized = email.trim().toLowerCase();
  return EMAIL_RE.test(normalized) ? normalized : null;
}

export function normalizeAndValidateCompanyCnpj(cnpj: string): string | null {
  const normalized = normalizeCnpj(cnpj);
  return isValidCnpj(normalized) ? normalized : null;
}

export function normalizeAndValidateUserCpf(cpf: string): string | null {
